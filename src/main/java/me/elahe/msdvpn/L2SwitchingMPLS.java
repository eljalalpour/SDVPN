		...
		if (event.type() == Type.HOST_ADDED) {
			...
			if (!vLanIdMap.containsKey(host.vlan())) {
				...
				for (Device d : deviceService.getAvailableDevices()) {
					...
					GroupDescription groupDescription = new DefaultGroupDescription(d.id(), GroupDescription.Type.ALL,
							buckets, gkey, gid.id(), appId);
					...
					// selectorBuilder.matchVlanId(host.vlan()).build();
					...
					/*
					 * Build flow rule based on our treatment and selector for
					 * table 1 because we want another rule for table 0
					 */
			...
			for (Host h : vLanIdMap.get(host.vlan())) {
				paths = topologyService.getPaths(topologyService.currentTopology(), deviceId, h.location().deviceId());
				...
				paths = topologyService.getPaths(topologyService.currentTopology(), h.location().deviceId(), deviceId);
			...
			treatmentBuilder1.setOutput(host.location().port());
			treatmentBuilder1.popMpls(EtherType.IPV4.ethType());
			TrafficTreatment treatment1 = treatmentBuilder1.build();
			GroupBucket bucket = DefaultGroupBucket.createAllGroupBucket(treatment1);
